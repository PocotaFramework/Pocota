...
// Cats.Contract.CatsContractControllerProxy
...
public class CatsContractControllerProxy : Controller
{
    ...
    public void FindCats(string? filter) 
    {
        ...
        filter1 =  filter is null
                    ? null
                    : JsonSerializer.Deserialize<ICatFilter?>(filter, jsonSerializerOptions);
    ...
    public void FindBreeds(string? filter) 
    {
        ...
        filter1 =  filter is null
                    ? null
                    : JsonSerializer.Deserialize<IBreedFilter?>(filter, jsonSerializerOptions);
        ICatsController contra = HttpContext.RequestServices.GetRequiredService<ICatsController>();
        ((Controller)contra).ControllerContext = ControllerContext;
    ...
    public void FindCatteries(string? filter) 
    {
        ...
        filter1 =  filter is null
                    ? null
                    : JsonSerializer.Deserialize<ICatteryFilter?>(filter, jsonSerializerOptions);
        ICatsController contra = HttpContext.RequestServices.GetRequiredService<ICatsController>();
        ((Controller)contra).ControllerContext = ControllerContext;
