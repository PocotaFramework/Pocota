...
#region Projection classes;
    ...
    public LitterProjection AsLitterProjection => As<LitterProjection>();
    public LitterForCatProjection AsLitterForCatProjection => As<LitterForCatProjection>();
    public LitterForDateProjection AsLitterForDateProjection => As<LitterForDateProjection>();
    public LitterWithCatsProjection AsLitterWithCatsProjection => As<LitterWithCatsProjection>();
    ...
    public override object? As(Type type)
    {
        ...
        {
            _asLitterProjection ??= new(this);
            return _asLitterProjection;
        }
        if(type == typeof(LitterForCatProjection) || type == typeof(ILitterForCat))
        {
            _asLitterForCatProjection ??= new(this);
            return _asLitterForCatProjection;
        }
        if(type == typeof(LitterForDateProjection) || type == typeof(ILitterForDate))
        {
            _asLitterForDateProjection ??= new(this);
            return _asLitterForDateProjection;
        }
        if(type == typeof(LitterWithCatsProjection) || type == typeof(ILitterWithCats))
        {
            _asLitterWithCatsProjection ??= new(this);
            return _asLitterWithCatsProjection;
        }
        return null;
    }
        ...
        protected virtual void StringsCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        if (e.OldItems is { })
        {
            foreach (INotifyPocoChanged item in e.OldItems)
            {
                item.PocoChanged -= StringsPocoChanged;
            }
        }
        if (e.NewItems is { })
        {
